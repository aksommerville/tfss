...
/* Callback from PulseAudio driver.
 */
...
static void cb_pcm_out(int16_t *v,int c,void *userdata) {
  ...
  int qbufsize=(QBUFLIMIT/g.chanc)*g.chanc; // Important that it be a multiple of chanc.
  while (c>0) {
    int updc=c;
    if (updc>qbufsize) updc=qbufsize;
    tfss_update(qbuf,updc);
    quantize_output(v,qbuf,updc);
    v+=updc;
    c-=updc;
  }
  #undef QBUFLIMIT
...
static int rcvarg(const char *k,int kc,const char *v,int vc) {
  ...
  if ((kc==4)&&!memcmp(k,"help",4)) {
    fprintf(stdout,
      ...
      "  --buffer=FRAMES  Set PulseAudio buffer size.\n"
  ...
  if ((kc==6)&&!memcmp(k,"buffer",6)) return rcvarg_uint(&g.buffer,v,vc,"buffer");
...
static int stream_update() {
  ...
  if (tmpc<=0) return -1;
  if (pulse_lock(g.pulse)<0) {
    fprintf(stderr,"%s: Failed to acquire driver's lock!\n",g.exename);
    return -2;
  }
  int tmpp=0;
  while (tmpp<tmpc) {
...
int main(int argc,char **argv) {
  ...
  /* Prepare driver.
   * This is a negotiation; it might change our rate or channel count.
   */
  struct pulse_delegate delegate={.pcm_out=cb_pcm_out};
  struct pulse_setup setup={
    .rate=g.rate,
    .chanc=g.chanc,
    .buffersize=g.buffer,
  };
  if (!(g.pulse=pulse_new(&delegate,&setup))) {
    fprintf(stderr,"%s: pulse_new failed. rate=%d chanc=%d buffersize=%d\n",g.exename,g.rate,g.chanc,g.buffer);
    return 1;
  }
  if ((g.pulse->rate!=g.rate)||(g.pulse->chanc!=g.chanc)) {
    fprintf(stderr,"%s:WARNING: PulseAudio changed request (%d,%d) to (%d,%d)\n",g.exename,g.rate,g.chanc,g.pulse->rate,g.pulse->chanc);
    g.rate=g.pulse->rate;
    g.chanc=g.pulse->chanc;
  }
   ...
   */
  if (g.src) {
    fprintf(stderr,"%s: Playing MIDI file '%s' via PulseAudio at %d hz and %d channels, buffer %d.\n",g.exename,g.srcpath,g.rate,g.chanc,g.buffer);
  } else {
    fprintf(stderr,"%s: Streaming events from '%s' into PulseAudio at %d hz and %d channels, buffer %d.\n",g.exename,g.srcpath,g.rate,g.chanc,g.buffer);
  }
  pulse_set_running(g.pulse,1);
  for (;;) {
    if (g.sigc) {
